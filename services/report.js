...
export const getOrderCount = async (token, query) => {
  const { data: response } = await axios.get(`service/report/order?${query}`, {
    headers: {
      "Token": token
    }
  })
...
export const invoicePayment = async (token, id) => {
  const { data: response } = await axios.get("/service/stripe/pay-invoice", {
    headers: {
      "Token": token
    },
    params: {
