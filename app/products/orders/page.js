...
export default function Orders() {
  ...
  const handleClear = async () => {
    ...
    setCompanyIds([])
    setTrash(false)
    const params = { trash: trash, for: "inventoryOrders" }
    getOrders(params)
  }
  ...
  useEffect(() => {
    const params = { status: ["Received"], trash: trash, for: "inventoryOrders" }
    getOrders(params)
    getCompanies()
  }, [active, trash])
          ...
          {orders.map((order) => {
            return (
              <TableRow key={order.id.toString()}>
                ...
                {userState.warehouseUser &&
                  <TableCell sx={{ color: constants.getCompanyStatus(order.company?.payment_status, order.company?.end_date) }}>
                    {order.company?.name}
                  </TableCell>}
                ...
                <TableCell align="center">
                  <IconButton color="primary" size="small" onClick={() => router.push(`/products/order/${order.id}`)}><AssignmentOutlined /></IconButton>
                  {userState.warehouseUser && (
                    !order.trash ? (
