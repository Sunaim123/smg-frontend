...
export default function Orders() {
  ...
  const [status, setStatus] = useState(searchParams.has("status") ? [searchParams.get("status")] : [])
  const [paymentStatus, setPaymentStatus] = useState(searchParams.has("payment_status") ? [searchParams.get("payment_status")] : [])
  ...
  useEffect(() => {
    const params = { trash: trash, from_date: filters.from_date, to_date: filters.to_date }
            ...
            return (
              <TableRow key={order.id.toString()}>
                ...
                <TableCell align="center">
                  <IconButton color="primary" size="small" onClick={() => router.push(`/sp/d/order/${order.id}`)}><AssignmentOutlined /></IconButton>
                  {userState.warehouseUser && (
                    !order.trash ? (
