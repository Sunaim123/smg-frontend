...
const Order = ({ params }) => {
  ...
  const fees = order?.order_lineitem.reduce((total, item) => { return total + (item.fees + item.other_fees) }, 0)
      ...
      <Container maxWidth="xl">
        <Box display="flex" justifyContent="space-between" alignItems="center" my={3}>
          ...
          <Box display="flex" gap={1}>
            {userState.companyUser && order.order_status === "received" && <Button variant="contained" size="small" disableElevation disabled={loading} onClick={() => router.push(`/sp/d/order/ship?id=${params.id}`)}>Mark as Shipped</Button>}
            {userState.customer && order.order_lineitem.every(x => x.status === "received") && <Button variant="contained" size="small" color="error" disableElevation disabled={loading} onClick={handleCancel}>Cancel Order</Button>}
            ...
            <Card variant="outlined">
              ...
              <CardContent>
                <Grid container spacing={1}>
                  ...
                  <Grid item md={6}><Typography variant="body2">${fees.toFixed(2)}</Typography></Grid>

                  <Grid item md={6}><Typography variant="body1" fontWeight={800}>YOU EARNED:</Typography></Grid>
                  <Grid item md={6}><Typography variant="body2">${(netTotal - fees).toFixed(2)}</Typography></Grid>
