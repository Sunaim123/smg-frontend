...
export default function Return({ params }) {
  ...
  const handleStripePayment = async (id) => {
    try {
      const response = await serviceApis.invoicePayment(userState.token, id)
      if (!response.status) throw new Error(response.message)

      if (response.url) router.push(response.url)
      setToast({ type: "success", open: true, message: "Invoice paid successfully" })
      ...
      <Container maxWidth="xl">
        <Box display="flex" justifyContent="space-between" alignItems="center" my={3} px={2}>
          ...
          <Box display="flex" gap={1}>
            {userState.companyUser && returnReceived && <Button variant="contained" onClick={() => router.push(`/sp/d/return/ship?id=${_return?.id}&title=Request Ship`)}>Request Ship</Button>}
            {userState.companyUser && returnReceived && <Button variant="contained" onClick={() => router.push(`/sp/d/return/service?id=${_return?.id}`)}>Add Service</Button>}
            {userState.warehouseUser && returnShipRequested && <Button variant="contained" onClick={() => handleClick(`/sp/d/return/ship?id=${_return?.id}&title=Mark as Shipped`)} disabled={loading}>Mark as Shipped</Button>}
          </Box>
        </Box>
              ...
              <TableBody>
                <TableRow key={`return-service-${service.id}`}>
                  ...
                  <TableCell align="center" sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                    ...
                    {userState.companyUser && service.payment_status !== "paid" && service.status !== "pending" && <Tooltip title="Pay Invoice"><IconButton onClick={() => handleStripePayment(service.invoice_id)} target="_blank" size="small"><ArrowForwardOutlined color="success" /></IconButton></Tooltip>}
                    {userState.warehouseUser && service.payment_status === "paid" && service.status === "requested" && <Tooltip title="Complete Service"><IconButton onClick={() => router.push(`/sp/${service.type}?id=${_return.id}`)}><ArrowForwardOutlined color="success" /></IconButton></Tooltip>}
