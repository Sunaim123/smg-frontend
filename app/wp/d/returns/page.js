...
import { Box, Grid, Button, Chip, Container, FormControl, IconButton, InputLabel, MenuItem, Select, Table, TableBody, TableCell, TableHead, TableRow, TextField, Tooltip, Typography, OutlinedInput } from "@mui/material"
    ...
    <>
      ...
      <Container maxWidth="xl">
        <Grid container justifyContent="space-between" alignItems="center" py={3}>
          ...
          <Grid item xs={10.5} container justifyContent="flex-end" alignItems="center" gap={1}>
            ...
            <Button variant="contained" onClick={() => handleLink("/wp/d/return")}>New</Button>
          </Grid>
        </Grid>
      </Container>
      <Table size="small">
            ...
            return (
              <TableRow key={_return.id.toString()}>
                ...
                <TableCell align="center" sx={{ display: "flex" }}>
                  {/* {userState.warehouseUser && _return.return_status === "ship requested" && <Tooltip title="Mark as Shipped" placement="top"><IconButton color="primary" onClick={() => handleLink(`/wp/status?id=${_return.id}&title=Mark as Shipped`)}><MoveToInbox /></IconButton></Tooltip>} */}
                  <Tooltip title="View" placement="top"><IconButton color="primary" onClick={() => handleLink(`/wp/d/return/${_return.id}`)}><AssignmentOutlined /></IconButton></Tooltip>
                  {userState.warehouseUser && <Tooltip title="Edit" placement="top"><IconButton color="primary" onClick={() => handleLink(`/wp/d/return?id=${_return.id}`)}><EditOutlined /></IconButton></Tooltip>}
                  {userState.warehouseUser && (
                    !_return.trash ? (
