    ...
    <>
      ...
      <Container maxWidth="xl">
        <Box display="flex" justifyContent="space-between" alignItems="center" my={3}>
          ...
          {userState.companyUser && <Button variant="contained" onClick={() => router.push("/wp/d/support")}>New</Button>}
        </Box>
      </Container>
          ...
          {feedbacks.map((feedback) => (
            <TableRow key={feedback.id.toString()}>
              ...
              <TableCell align="center">
                <IconButton color="primary" onClick={() => router.push(`/wp/d/support/${feedback.id}`)}><AssignmentOutlined color="primary" /></IconButton>
                {feedback.status !== "Closed" && <IconButton color="primary" onClick={() => router.push(`/wp/d/support?id=${feedback.id}`)}><EditOutlined color="primary" /></IconButton>}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
