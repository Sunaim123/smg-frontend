      ...
      <Container maxWidth="xl">
        <Grid container justifyContent="space-between" alignItems="center" py={3}>
          ...
          <Grid item xs={8} container justifyContent="flex-end" alignItems="center" gap={1}>
            ...
            <Button variant="contained" color="error" onClick={handleFilter}>Clear</Button>
            {userState.warehouseUser && <Button variant="contained" onClick={() => handleLink("/wp/d/invoice")}>New</Button>}
          </Grid>
        </Grid>
      </Container>
      <Table size="small">
            ...
            return (
              <TableRow key={invoice.id.toString()}>
                ...
                <TableCell align="right"  sx={{ display: "flex" }}>
                  {userState.warehouseUser && !isTrash && isAwaitingPayment && <Tooltip title="Edit" placement="top"><IconButton color="primary" onClick={() => handleLink(`/wp/d/invoice?id=${invoice.id}`)}><EditOutlined /></IconButton></Tooltip>}
                  {!invoice.trash && <Tooltip title="View" placement="top"><IconButton color="primary" onClick={() => handleLink(`/wp/d/invoice/${invoice.id}`)}><AssignmentOutlined /></IconButton></Tooltip>}
                  {userState.warehouseUser && (
                    !isTrash ? (
