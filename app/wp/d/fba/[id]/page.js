...
export default function Fba({ params }) {
  ...
  useEffect(() => {
    if (userState.customer) router.replace("/wp/d/products")
            ...
            <Grid item xs={4}>
              <Box display="flex" justifyItems="center" alignItems="center" gap={2}>
                {(userState.warehouseUser && fba.payment_status === "paid") && <>
                  {!fba.received && !fba.shipped && <Button variant="contained" size="small" disableElevation onClick={() => router.push(`/wp/d/fba/receive?id=${fba?.id}`)}>Received</Button>}
